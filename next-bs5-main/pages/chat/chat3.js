...
export default function Chat() {
    ...
    if (userID) {
      ...
      socket.addEventListener('message', (event) => {
        ...
        if (result.type === 'registered') {
          ...
          const userID = result.otherNames.userID
        ...
        if (result.type === 'message') {
          ...
          const newMessage =
            result.fromName === userName
              ? `<div class='d-flex justify-content-end align-items-center mb-1 user-font'>${testCreate}</div>`
              : `<div class='d-flex align-items-center mb-1 user-font'><img class="userimg" src="http://localhost:3005/avatar/${fromID}.png"/>${fromName}${testCreate}</div>`
  ...
  const sendMessage = () => {
    ...
    if (testTarget) {
      prarms.targetUserId = testTarget
    }
